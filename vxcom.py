...
# ----------------- CONFIGURATION -----------------
BOT_TOKEN = "8141174859:AAEWdoTYm9cu9abv6nLuhY4MUQHA8GzI1yw"
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot Engine by shivu-vxcom is Active 24/7")
...
def send_delayed_give_hit(target_id):
    ...
    try:
        bot.send_message(target_id, "𝗚𝗜𝗩𝗘 𝗛𝗜𝗧 𝗢𝗥 𝗠𝗔𝗜𝗟 𝗔𝗡𝗗 𝙔O𝙐𝗥 𝗣𝗔𝗦𝗦", reply_markup=cancel_kb, protect_content=True)
    except Exception:
        pass
...
def handle_all_messages(message):
    ...
    if is_admin(user_id) and user_id in user_states:
        ...
        if state_data == "ADMIN_MANAGE_CREDITS_ID":
            ...
            bot.send_message(user_id, f"⏳ Broadcasting message to {len(all_users)} registered users...")
            sent, failed = 0, 0
            for uid in all_users:
                try:
                    if message.content_type == 'photo':
                        bot.send_photo(int(uid), message.photo[-1].file_id, caption=message.caption or "", protect_content=True)
                    else:
                        bot.send_message(int(uid), text, protect_content=True)
                    sent += 1
                except Exception:
                    failed += 1
    ...
    if text == "🛠️ Admin Panel" and is_admin(user_id):
        ...
        if qr_to_send:
            try:
                bot.send_photo(user_id, qr_to_send, caption=caption_text, protect_content=True)
                bot.send_message(user_id, second_text, reply_markup=cancel_kb, protect_content=True)
                return
            except Exception:
                pass
        
        bot.send_message(user_id, f"{caption_text}\n\n{second_text}", reply_markup=cancel_kb, protect_content=True)
